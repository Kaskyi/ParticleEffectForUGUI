...
namespace Razomy.Unity.UIParticles.Scripts.Editor
{
  ...
  internal class UIParticleEditor : GraphicEditor
  {
    ...
    private class UIParticleOverlay : IMGUIOverlay, ITransientOverlay
    {
      ...
      {
        if (visible) WindowFunction(null, null);
      }
    }
#endif
    ...
    private static bool _xyzMode;
    private bool _showMax;

    private static readonly HashSet<Shader> s_Shaders = new();
    private static readonly List<ParticleSystemVertexStream> s_Streams = new();
 ...
 _ => { if (s_SerializedObject != null) miSceneViewOverlayWindow.Invoke(null, sceneViewArgs); };
#else
    ...
    public override void OnInspectorGUI()
    {
      ...
      if (FixButton(current.m_IsTrail,
            "This UIParticle component should be removed. The UIParticle for trails is no longer needed."))
      ...
      if (0 < allPsRenderers.Length)
      {
        ...
        if (FixButton(sp.boolValue || sp.hasMultipleDifferentValues,
              "When using linear color space, the particle colors are not output correctly.\nTo fix, set 'Apply Active Color Space' in renderer module to false."))
        {
          sp.boolValue = false;
