...
namespace Razomy.Unity.UIParticles.Scripts
{
  ...
  public class UIParticle : MaskableGraphic
  {
    ...
    [Tooltip(
      "Animatable material properties. If you want to change the material properties of the ParticleSystem in Animation, enable it.")]
    [SerializeField]
    ...
    private readonly List<UIParticleRenderer> m_Renderers = new();
    ...
    internal bool isPrimary => m_MeshSharing == MeshSharing.Primary || m_MeshSharing == MeshSharing.PrimarySimulator;

    internal bool canSimulate => m_MeshSharing == MeshSharing.None || m_MeshSharing == MeshSharing.Auto ||
                                 m_MeshSharing == MeshSharing.Primary || m_MeshSharing == MeshSharing.PrimarySimulator;

    internal bool canRender => m_MeshSharing == MeshSharing.None || m_MeshSharing == MeshSharing.Auto ||
                               m_MeshSharing == MeshSharing.Primary || m_MeshSharing == MeshSharing.Reprica;
    ...
    public void RefreshParticles(List<ParticleSystem> particles)
    {
      ...
      for (var i = 0; i < particles.Count; i++)
      {
        ...
        if (particles[i].trails.enabled) GetRenderer(j++).Set(this, particles[i], true);
      }
    ...
    internal void UpdateRenderers()
    {
      ...
      if (m_Renderers.Any(x => !x)) RefreshParticles(particles);
    ...
    {
      if (m_Renderers.Count <= index) m_Renderers.Add(UIParticleRenderer.AddRenderer(this, index));
