...
namespace Razomy.Unity.UIParticles.Scripts
{
  ...
  internal class UIParticleRenderer : MaskableGraphic
  {
    ...
    public void UpdateMesh(Camera bakeCamera)
    {
      ...
      if (!_isTrail && _parent.canSimulate)
      {
...
#endif
        {
          ...
          if (!main.loop && main.duration <= _particleSystem.time &&
              (_particleSystem.IsAlive() || _particleSystem.particleCount == 0)) _particleSystem.Stop(false);
        }
    ...
    private void ResolveResolutionChange(Vector3 psPos, Vector3 scale)
    {
      ...
      if ((_prevScreenSize != screenSize || _prevScale != scale) &&
    ...
    private void Simulate(Vector3 scale, bool paused)
    {
      ...
      var rateOverDistance = emission.enabled && 0 < emission.rateOverDistance.constant &&
    ...
    private void SimulateForEditor(Vector3 diffPos, Vector3 scale)
    {
      ...
      if (_particleSystem.main.simulationSpace == ParticleSystemSimulationSpace.World &&
          0 < Vector3.SqrMagnitude(diffPos))
      {
    ...
    private void UpdateMaterialProperties()
    {
      ...
      foreach (var ap in _parent.m_AnimatableProperties) ap.UpdateMaterialProperties(_modifiedMaterial, s_Mpb);
